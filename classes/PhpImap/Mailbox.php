...
class Mailbox {
	...
	{
		return $this->imap('UID SEARCH',['UID',$uid,$this->serverEncoding])?:[];
	}
	/**
	 * Save mail body.
	 * @param $mailId
	...
	protected function convertStringEncoding($string, $fromEncoding, $toEncoding) {
		...
		if(!$convertedString) {
			die ('Mime string encoding conversion failed');
		}
		...
		if(!$result) {
			...
			if($errors) {
				if($throwExceptionClass) {
					die("IMAP method imap_$methodShortName() failed with error: " . implode('. ', $errors));
				}
				else {
					return false;
				}
